var rpt_amortunearnedinterest = function(){
	return{
		xtype:'form'
		,id:'rpt_amortunearnedinterest'
		,region: 'center'
		,anchor: '100%'
		,autoscroll: true
		,frame: true
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_amortunearnedinterest').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
		    			url: '/report_amortunearnedinterest' 
						,method: 'POST'
						,form: Ext.get('frmDownload')
						,params: {'file_type': Ext.getCmp('rpt_amortunearnedinterest').getForm().getValues().file_type
									,'report_date': Ext.getCmp('rpt_amortunearnedinterest').getForm().getValues().report_date
									,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'Amortization of Unearned Interest Report'
	    	,layout: 'form'
            ,anchor: '50%'
            ,items: [{
                xtype: 'datefield'
