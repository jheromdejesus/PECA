	...
	[
		...
		,{header: 'Interest Amortization', width: 100, sortable: true, align: 'right', dataIndex: 'interest_amortization', 
			...
			, summaryRenderer: function(v, params, data){
				...
				record = pecaDataStores.memberLoanInfoSumStore.queryBy(function(record,id){ 
					return parseFloat(record.get('principal_balance'))>0;
				}); 
				var sum_interest = 0;
				record.each(function(item,index){ 
					sum_interest += parseFloat(item.get('interest_amortization'));
				});
				return Ext.util.Format.number(sum_interest,'0,000,000,000.00'); }
...
*/
		...
		,{header: 'Principal Amortization', width: 100, sortable: true, align: 'right', dataIndex: 'principal_amortization', 
			...
			, summaryRenderer: function(v, params, data){
				...
				var sum_principal = 0;
				record.each(function(item,index){ 
					sum_principal += parseFloat(item.get('principal_amortization'));
				});
				return Ext.util.Format.number(sum_principal,'0,000,000,000.00'); }
/*
				if(pecaDataStores.memberLoanInfoSumStore.getCount()>0)
...
*/
		...
		,{header: 'Monthly Amortization', width: 100, sortable: true, align: 'right', dataIndex: 'montly_amortization', 
			...
			, summaryRenderer: function(v, params, data){
				...
				var sum_monthly = 0;
				record.each(function(item,index){ 
					sum_monthly += parseFloat(item.get('monthly_amortization'));
				});
...
*/
		...
		,{header: 'Principal Balance', width: 100, sortable: true, align: 'right', dataIndex: 'principal_balance', 
			...
			, summaryRenderer: function(v, params, data){
				...
				record.each(function(item,index){ 
					sum_balance += parseFloat(item.get('principal_balance'));
				});

				return Ext.util.Format.number(sum_balance,'0,000,000,000.00'); 
...
var memberDetail = function(){
	return {
	    ...
	    ,setModeNew: function() {
			Ext.getCmp('memInfoTab_status').setValue("I");
			if(_GROUP_ID == "OPERATIONS"){
				Ext.getCmp('memInfoTab_status').setValue("I");
				Ext.getCmp('memInfoTab_status').addClass("x-item-disabled");
				Ext.getCmp('memInfoTab_status').setReadOnly(true);
				Ext.getCmp('memInfoTab_status').label.update('Status <span style=\"color:red;\"> * </span>');
			}
		...
		,items:[{
			...
			,items: [{
				...
				,items:[{
					...
					xtype: 'hidden'
					,submitValue: false
					,name: 'member[email_info_permission]'
					,id: 'email_permission'
					...
					,validator: function(){
						...
						if (document.getElementById('email_permission') != null){
							emailpermission = document.getElementById('email_permission').value;
						} else{
							return true;
						}
						if(emailpermission != 'true') {
							document.getElementById('email_add').readOnly = true;
							document.getElementById("email_add").style.color="#ADADAD";
						} else {
							document.getElementById('email_add').readOnly =  false;
							document.getElementById("email_add").style.color="#000000";
						}
						return true;
					}
				}]
...
var bankInfoTab= function(){
	return {
		...
		,items:[{
				...
				xtype: 'hidden'
				,submitValue: false
				,name: 'member[bank_info_permission]'
				,id: 'bank_permission'
			}
			,{
				xtype: 'textfield'
				...
				,validator: function(){
					...
					if (document.getElementById('bank_permission') != null){
						permission = document.getElementById('bank_permission').value;
					} else{
						return true;
					}
					if(permission != 'true') {
						...
						document.getElementById("bank_account_no").style.color="#ADADAD";
						document.getElementById("bank").style.color="#ADADAD";
					} else {
						document.getElementById('bank_account_no').readOnly =  false;
						document.getElementById('bank').readOnly = false;
						document.getElementById("bank_account_no").style.color="#000000";
						document.getElementById("bank").style.color="#000000";
					}
					return true;
				}
	...
	return {
		...
		,items:[{
			...
			,items: [{
				...
				,columnWidth: .5
				// jdj 08312017
				//changed the padding to fit the highest capital and date
				...
				,items: [{
					...
					//[START] Modified by Vincent Sy 8th Enhancement 2013/07/03
					,fieldLabel: 'Non-Withdrawable Capital'			
					//[END] Modified by Vincent Sy 8th Enhancement 2013/07/03
				},{
					html: '<br />'
				},{
					xtype: 'textfield'
					...
					,width: 120
					,style: 'background:transparent; text-align: right; border:0'
					,id: 'member[capcon111date]'
					,fieldLabel: 'Highest Capital Date'
						// end		
...
/* ##### NRB EDIT START ##### */
var memberLoanWin = function(b_bsp_computation){
...
/* ##### NRB EDIT END ##### */
	return new Ext.Window({
		...
		,loadMask: true
		/* ##### NRB EDIT START ##### */
		,items:[ memberLoanDetails(b_bsp_computation) ]
...
var memberLoanDetailTab = function(b_bsp_computation){
	/*return {
		xtype: 'tabpanel'
		,titlebar:false
		,activeTab: 0
...
/* ##### NRB EDIT START */
var memberLoanDetails = function(b_bsp_computation) {
	if(b_bsp_computation) {
		return{
		xtype: 'form'
		...
		,items:[{	
			...
			,items:[{
				...
				,items:[{
						...
						,readOnly: true
						,submitValue: false
						,name: 'effective_monthly_interest_rate'
						,anchor: '95%'
						,style: 'background:transparent; text-align: right; border:0'
		...
		}, memberLoanDetailTab(b_bsp_computation)]
		}
	} else {
		return{
		xtype: 'form'
				...
				,fn:function(grid, row, e) {
					...
					Ext.getCmp('memberDetail').getForm().load({
						...
						,success: function(response, opts) {
							...
							Ext.Ajax.request({
								//[START] Modified by Vincent Sy for 8th Enhancement 2013071
								url: '/membership/showMembershipInfoInTransHist'
								//[END] Modified by Vincent Sy for 8th Enhancement 20130710
								,params: {'member[employee_id]':(rec.get('member[employee_id]'))
											,auth:_AUTH_KEY}  
								,success: function(response, opts) {
									...
									//jdj 08032017
									//retrieves the highest capital and the date(month and year)
									Ext.getCmp('member[capcon111]').setValue(Ext.util.Format.number(Ext.num(ret.data.capcon11,0),'0,000,000,000.00'));
									Ext.getCmp('member[capcon111date]').setValue(ret.data.capcon11date);
									//end 08312017
			...
			'rowdblclick':{
				...
				,fn:function(grid, row, e) {
					...
					if(rec.get('bsp_computation') == 'Y') {
						memberLoanWin(true).show();
					} else {
						memberLoanWin(false).show();
					}
					/* memberLoanWin().show(); */
					/* ##### NRB EDIT END ##### */
...
//[START] 7th Enhancement
var memberAmortizationScheduleTab= function(){
	return {
		xtype:'panel'
		,title:'Amortization Schedule'
		,id: 'amortization_schedule_tab'
		,bodyStyle: 'background:transparent padding: 5px;'
		,items: [rpt_amortizationschedule("member_loan_info_loan_no")]
	};
};
//[END] 7th Enhancement
