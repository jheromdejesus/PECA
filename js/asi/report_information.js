	...
	return{
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_information').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
		    			url: '/report_information' 
						,method: 'POST'
						,form: Ext.get('frmDownload')
						,params: {file_type: Ext.getCmp('rpt_information').getForm().getValues().file_type
			        				,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'Information Code Summary'
	    	,layout: 'form'
            ,anchor: '50%'
            ,items: [{
            	xtype: 'radiogroup'
