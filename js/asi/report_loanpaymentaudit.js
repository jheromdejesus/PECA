	...
	return{
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_lpa').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
						...
						,params: {'file_type': Ext.getCmp('rpt_lpa').getForm().getValues().file_type
									,'report_date': Ext.getCmp('rpt_lpa').getForm().getValues().report_date
									,'report_type': Ext.getCmp('rpt_lpa').getForm().getValues().report_type
									,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'Loan Payments Audit Trail Report'
	    	,layout: 'form'
            ,anchor: '50%'
            ,items: [{
                xtype: 'datefield'
