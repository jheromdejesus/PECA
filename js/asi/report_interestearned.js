var rpt_interestearned = function(){
	return{
		xtype:'form'
		,id:'rpt_interestearned'
		,region: 'center'
		,anchor: '100%'
		,autoscroll: true
		,frame: true
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_interestearned').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
		    			url: '/report_interestearned' 
						,method: 'POST'
						,form: Ext.get('frmDownload')
						,params: {'file_type': Ext.getCmp('rpt_interestearned').getForm().getValues().file_type
									,'month': Ext.getCmp('rpt_interestearned').getForm().getValues().month
									,'year': Ext.getCmp('rpt_interestearned').getForm().getValues().year
									,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'List of Interest Earned Report'
	    	,layout: 'form'
            ,anchor: '70%'
            ,items: [
            {
