...
var ol_withdrawalColumns =  new Ext.grid.ColumnModel( 
	[
		...
		,{header: 'Time Sent', width: 80, sortable: true, dataIndex: 'online_withdrawal[time_sent]',align:'center'}
				...
				,{
					...
					,items: [
						...
						,{
							...
							,anchor: '95%'
							,hiddenName: 'online_withdrawal[transaction_date]'
							,id: 'subD'
			...
			if(Ext.getCmp('ol_withdrawalDetail').getForm().findField('ow_status').getValue()==2){
				frm.submit({
					...
					,success: function(form, action) {
						...
						frm.findField('ow_status').setValue(value);
						frm.setModeUpdate();
						//[start] google share#7 added by asi466
						Ext.getCmp('ol_withdrawalDetail').getForm().load({
							url: '/online_withdrawal/showHeader'
							,params: {'request_no':(action.result.request_no)
										,auth:_AUTH_KEY}
						...
						//[end] google share#7 added by asi466
					}
					,failure: function(form, action) {					
						//20111118 commented by ASI466 because this(prompt) is misleading to the user 
						//showExtErrorMsg(action.result.msg);									
			...
			if(frm_.isValid()){
				...
				Ext.Ajax.request({
					...
					,params: {'employee_id': frm_.findField('online_withdrawal[employee_id]').getValue()
							...
							,'transaction_date': frm_.findField('online_withdrawal[transaction_date]').getValue()
