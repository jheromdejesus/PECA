...
Ext.extend(Ext.App, Ext.util.Observable, {
    ...
    STATUS_OK:                  "ok",
    STATUS_HELP:                "help",
    STATUS_ERROR_MESSAGE:       "Error Message",
    STATUS_OK_MESSAGE:          "Success Message",
    ...
    buildMessageBox : function(title, msg) {
        switch (title) {
            case true:
                title = this.STATUS_OK_MESSAGE;
                break;
            case false:
                title = this.STATUS_ERROR_MESSAGE;
                break;
        }
        return [
            '<div class="app-msg">',
            '<div class="x-box-tl"><div class="x-box-tr"><div class="x-box-tc"></div></div></div>',
            '<div class="x-box-ml"><div class="x-box-mr"><div class="x-box-mc">', title, '<h3 class="x-icon-text icon-status-' + title + '">', msg, '</h3></div></div></div>',
...
Ext.data.DataProxy.addListener('exception', function(proxy, type, action, options, res) {
	if(type == 'remote'){
		try{
			App.setAlert(false, res.raw.msg);
		}catch(e){
			location.reload();
		}
	}else if(type == 'response'){
		try{
			if( Ext.decode(res.responseText).error_code != null && Ext.decode(res.responseText).error_code == '-1'){
				App.setAlert(false, Ext.decode(res.responseText).msg);
			}else{
				App.setAlert(true, Ext.decode(res.responseText).msg);
			}
		}catch(e){
			location.reload();
		}
