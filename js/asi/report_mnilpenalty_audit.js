var rpt_mnilpenalty_audit = function(){
	return{
		xtype:'form'
		,id:'rpt_mnilpenalty_audit'
		,region: 'center'
		,anchor: '100%'
		,autoscroll: true
		,frame: true
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_mnilpenalty_audit').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
		    			url: '/report_mnilpenaltyaudit' 
						,method: 'POST'
						,form: Ext.get('frmDownload')
						,params: {'file_type': Ext.getCmp('rpt_mnilpenalty_audit').getForm().getValues().file_type
									,'report_date': Ext.getCmp('rpt_mnilpenalty_audit').getForm().getValues().report_date
									,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'Mini Loan Penalty Audit Report (PENT)'
	    	,layout: 'form'
            ,anchor: '50%'
            ,items: [{
                xtype: 'datefield'
