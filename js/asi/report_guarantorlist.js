var rpt_guarantorlist = function(){
	return{
		xtype:'form'
		,id:'rpt_guarantorlist'
		,region: 'center'
		,anchor: '100%'
		,autoscroll: true
		,frame: true
		...
		,buttons:[{
		    ...
		    ,handler : function(btn){
				var frm = Ext.getCmp('rpt_guarantorlist').getForm();
		    	if(frm.isValid()){
		    		try {
		    		...
		    		Ext.Ajax.request({
		    			url: '/report_guarantorlist' 
						,method: 'POST'
						,form: Ext.get('frmDownload')
						,params: {file_type: Ext.getCmp('rpt_guarantorlist').getForm().getValues().file_type
									,report_date: Ext.getCmp('rpt_guarantorlist').getForm().getValues().report_date
			        				,auth:_AUTH_KEY}
						,isUpload: true
	    ...
	    ,items: [{
	    	xtype: 'fieldset'
	    	,title: 'List of Comakers for Review Report'
	    	,layout: 'form'
            ,anchor: '50%'
            ,items: [{
                xtype: 'datefield'
