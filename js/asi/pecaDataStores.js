...
var pecaReaders ={
	...
	,loancodeHdrReader: new Ext.data.JsonReader({
		...
		,root: 'data'
		    ...
		    ,{name: 'loancodeHdr[bsp_sbl]', mapping: 'bsp_sbl', type: 'string', convert: function(value, rec){
		    	return formatCheckbox(value);}}
		    /**#####**/
		    ,{name: 'loancodeHdr[bsp_computation]', mapping: 'bsp_computation', type: 'string', convert: function(value, rec){
		    	return formatCheckbox(value);}}
		    /**#####**/	
	...
	,newloanReader: new Ext.data.JsonReader({
	    ...
	    ,root: 'data'
			...
			,{name: 'newloan[comaker_cb]', mapping: 'comaker_cb', type: 'string', convert: function(value, rec){
			    return formatCheckbox(value);}}
			/***** NRB EDIT START *****/
			,{name: 'newloan[effective_annual_interest_rate]', mapping: 'effective_annual_interest_rate', type: 'string'}
			,{name: 'newloan[effective_monthly_interest_rate]', mapping: 'effective_monthly_interest_rate', type: 'string'}
			,{name: 'newloan[mri_fip_provider]', mapping: 'mri_fip_provider', type: 'string'}
			,{name: 'newloan[other_charges_amount]', mapping: 'other_charges_amount', type: 'string'}
			,{name: 'newloan[other_charges_rate]', mapping: 'other_charges_rate', type: 'string'}
			/***** NRB EDIT END *****/
	])
	...
	,lpLoanListReader: new Ext.data.JsonReader({
	    ...
	    ,root: 'data' 	
			...
			,{name: 'bsp_computation', mapping: 'bsp_computation', type: 'string'}
			//[END] 7th Enhancement
	])
	//Investment Reader
	...
	/***** NRB EDIT START *****/
	//For Entry Type Dropdown
	,entryDetailFieldNameReader: new Ext.data.JsonReader({
	    totalProperty: 'total'
	    ,root: 'data' 	
	    },[
		    {name: 'account_no', mapping: 'account_no', type: 'string'}
	...
	,ol_loanReader: new Ext.data.JsonReader({
		...
		,root: 'data'
			...
			,{name: 'online_loan[time_sent]', mapping: 'time_sent', type: 'string'}	
	...
	,ol_withdrawalReader: new Ext.data.JsonReader({
		...
		,root: 'data'
		    ...
		    {name: 'online_withdrawal[transaction_date]', mapping: 'transaction_date', type: 'date', convert:function(value,rec){
				return formatDate(value);}}
			//Added for 6th Enhancement
			,{name: 'online_withdrawal[time_sent]', mapping: 'time_sent', type: 'string'}	
	...
	,memberReader: new Ext.data.JsonReader({
		...
		,root: 'data'
			...
			,{name: 'member[bank_info_permission]', mapping: 'bank_info_permission', type: 'string'}
			,{name: 'member[email_info_permission]', mapping: 'email_info_permission', type: 'string'}
	    ]
	)
	...
	,memberLoanInfoDetailReader: new Ext.data.JsonReader({
		...
		,root: 'data'
			...
			,{name: 'fip_due_date', mapping: 'fip_due_date', type: 'string', convert:function(value,rec){
				return formatDate(value);}}
			/* ##### NRB EDIT START ##### */
			,{name: 'effective_annual_interest_rate', mapping: 'effective_annual_interest_rate', type: 'string', convert: function(value, rec){
		    	return Ext.util.Format.number(value,'0,000,000,000.00');}}
			,{name: 'effective_monthly_interest_rate', mapping: 'effective_monthly_interest_rate', type: 'string', convert: function(value, rec){
		    	return Ext.util.Format.number(value,'0,000,000,000.00');}}
			,{name: 'mri_fip_provider', mapping: 'mri_fip_provider', type: 'string'}
			,{name: 'other_charges_amount', mapping: 'other_charges_amount', type: 'string', convert: function(value, rec){
		    	return Ext.util.Format.number(value,'0,000,000,000.00');}}
			,{name: 'other_charges_rate', mapping: 'other_charges_rate', type: 'string', convert: function(value, rec){
		    	return Ext.util.Format.number(value,'0,000,000,000.00');}}
			/* ##### NRB EDIT END ##### */
		]
	)
...
var pecaDataStores = {
	...
	/***** NRB EDIT START *****/
	//GL Entry MRI FIP Field Name Store
	,glEntryFieldNameStore: new Ext.data.Store({
		//restful: true
	    url: '/gl_entries/readDtlFieldName'
	    ,reader: pecaReaders.entryDetailFieldNameReader
	    ,baseParams: {auth:_AUTH_KEY}
	})
	/***** NRB EDIT END *****/
