...
class Subsidyreport_model extends Asi_Model {
	...
	function readAll($innerJoin =null, $select =null, $orderby = null){
		...
		$this->db->group_by('m_loan.employee_id');
	...
	/**
	 * jdj 09202017: for subsidy breakdown report
	
	 */
	function readAll2($innerJoin =null, $select =null, $orderby = null){
		if($select)
			$this->db->select($select);
			$this->db->from($this->table_name);
			$this->db->join('( '.$innerJoin.' ) L', 'L.loan_no=m_loan.loan_no ', 'INNER');
