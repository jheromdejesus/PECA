...
class Loan extends Asi_Controller {
	...
	function deleteMemberComaker(){
		log_message('debug', "[START] Controller loan:deleteMemberComaker");
		$_REQUEST['data'] = '"' . $_REQUEST['loan_no'] . ':' . $_REQUEST['employee_id'] . '"';
		//20110805 fix for unsync co-maker admin and co-maker online 
		$this->transferComakerToDelete();
		$sql1 = "SELECT me.`suspended_date`
				...
				WHERE mlg.`guarantor_id`='01517240'";
		...
		if($this->member_model->getValue('invalid_date')=='' || $this->member_model->getValue('invalid_date')==null ){
			$deletesuspension='99991231';
			$this->member_model->setValue('suspended_date',$deletesuspension);
			$this->member_model->update(array('employee_id' => $sql1));
		}
		
		$this->deleteComaker(true);
		log_message('debug', "[END] Controller loan:deleteMemberComaker");	
	}
