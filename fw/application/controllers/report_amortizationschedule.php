...
/*
 * Created on 2012/08/20 - PECA 7th Enhancement - Amortization Schedule Generation
 * @author Reynaldo P. Libutan
 * 
 */
 //require realpath($_SERVER["DOCUMENT_ROOT"]).'/fw/application/my_classes/Classes/PHPExcel/Calculation/Functions.php';
 require BASEPATH.'application/my_classes/Classes/PHPExcel/Calculation/Functions.php';
 
 class Report_amortizationschedule extends Asi_Controller {
	
	function Report_amortizationschedule(){
	...
	function index() {	
		...
		if (!isset($_REQUEST['loan_no'])) {	
			...
			$data = $this->getLoanAmortizationScheduleReportParameters($_REQUEST["loan_no"]);
			$display = $data;
			
			$this->initializeData($data);
			
			$this->computeFromPeriod($data);
			...
			$data["payroll_deduction"] = $this->getPayrollDeductionField($data["loan_code"]);
			
			//get employee name for display
			$employee_full_name = $this->getEmployeeName($data["employee_id"]);
			...
			if($data["payroll_deduction"] === "N") {
				...
				$this->generateGuideSchedule($data);
			}
			//else, consider previous payments in generating the report
				...
				$loan_payments = $this->getLoanPayments($_REQUEST["loan_no"]);
				...
				$data["balance"] = $this->getBalanceAtPeriod($data["principal"], $loan_payments, $first_period_of_current_loan_year);
				//get outstanding balance for this period (month)
				$data["current_balance"] = $this->getBalanceAtPeriod($data["principal"], $loan_payments, date("Ymd", $data["current_date_without_time"]));
				
				//true when user attempts to generate within loan duration but loan is already paid full.
				if($data["balance"] == 0) {
					echo("{'success':false,'msg':'Cannot generate report when outstanding loan balance is zero.','error_code':'19'}");
				...
				$this->generateActualSchedule($loan_payments, $data, $display);
			}
					
			$this->computeMIRandEIR($data);
			
			$this->formatDisplay($data, $display);
			$this->printPDF($data, $display);
	...
	private function getLoanAmortizationScheduleReportParameters($loan_no) {		
	...
	private function getCurrentDateFromSysParam() {
		$current_date_param_name = "CurrentDate";
		$data = $this->parameter_model->get(
	...
	private function printPDF($data, $display) {	
	...
	private function getLoanPayments($loan_no) {		
		$data = $this->mloanpayment_model->get_list(
			array('loan_no' => $loan_no)
			,null
			,null
			,array(
	...
	private function getBalanceAtPeriod($initial_balance, $loan_payments, $period) {
	...
	private function getTotalPaymentMadeAtPeriod($loan_payments, $fromPeriod, $toPeriod) {
		$total_payment = "0.00";
		foreach($loan_payments as $payment) {
	...
	private function getPayrollDeductionField($loan_code) {
		$data = $this->loancodeheader_model->get(
	...
	private function computeFromPeriod(&$data) {
		$period = 0;
		$fromPeriod = -1;
		...
		//during amortization
			...
			$period = $this->dateDifference($amortization_startdate, $current_date);
	...
	private function initializeData(&$data) {
		...
		$data["current_date"] = date("F d, Y G:i:s", strtotime($this->getCurrentDateFromSysParam()));
	...
	private function generateGuideSchedule(&$data) {
	...
	private function generateActualSchedule($loan_payments, &$data, &$display) {
		$periods = null;
		...
		for($i = $data["fromPeriod"], $period_counter = $data["fromPeriod"]; /*$i <= $data["term"] && */floor($data["balance"]) != 0; $i++, $period_counter++) {
			...
			if($period_counter < $data["period"]) {
				...
				$total_payment = intval($this->getTotalPaymentMadeAtPeriod($loan_payments, $payment_from, $payment_to));
				$payments = $this->getPaymentsMadeAtPeriod($loan_payments, $payment_from, $payment_to);
					
				$this->computePastTerm($i, $period_counter, $periods, $data, $total_payment, $payments);
			}
			//if during current period
				...
				$this->computeDuringTerm($i, $period_counter, $periods, $data, $total_payment, $payments);
			}		
			//after current period
			else {						
				$this->computeFutureTerm($i, $period_counter, $periods, $data);
			}
	...
	private function computeMIRandEIR(&$data) {
		//compute MIR
	...
	private function formatDisplay(&$data, &$display) {	
		$display["loan_period"] = $this->addOrdinalSuffix(((($data["fromPeriod"]-1)/12)+1))." Loan Year";
	...
	private function computePastTerm(&$i, $period_counter, &$periods, &$data, $total_payment, $payments) {
		if(count($payments) === 0) {
			//no payments
			$period_principal = 0;				
		...
		foreach($payments as $payment) {
			// Modified for Incorrect Interest Amount Issue by ASI372 2013/05/10
	...
	private function computeDuringTerm(&$i, $period_counter, &$periods, &$data, $total_payment, $payments) {
		if(count($payments) === 0) {
			//no payments
	...
	private function computeFutureTerm(&$i, $period_counter, &$periods, &$data) {
		$periods[$i]["period"] = $period_counter;
		$interest = $data["balance"] * ($data["interest_rate"] / 100) / 12;
		if($period_counter <= $data["toPeriod"]) {
			$data["total_interest_paid"] += round($interest);
	...
	private function getPaymentsMadeAtPeriod($loan_payments, $fromPeriod, $toPeriod) {
		$payments = array();
		foreach($loan_payments as $payment) {
			if($payment["payment_date"] >= $fromPeriod && $payment["payment_date"] < $toPeriod) {
