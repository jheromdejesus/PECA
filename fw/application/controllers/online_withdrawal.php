 ...
 */
 ...
 class Online_withdrawal extends Asi_Controller {
	...
	function readHeader()
	{
	    ...
	    $data = $this->Onlinecapitaltransactionheader_model->retrieveAllOnlineCapTransactions(
			...
			,array('tc.request_no AS request_no'
				...
				//Added for PECA 6th Enhancement Kweeny Libutan 2012/05/09
				,'tc.time_sent AS time_sent'
	...
	function showHeader()
	{
		...
		$data = $this->Onlinecapitaltransactionheader_model->retrieveOnlineCapTransactionHeader(
				...
				,array('tc.request_no AS request_no'
						...
						,'ow.approver4 AS approver4'
						,'ow.approver5 AS approver5'
						//[Start]modified by asi466 on issue#0008402
						,'tc.withdrawable_amount AS maxWdwlAmount'
						//[End]modified by asi466 on issue#0008402
						)
		...
		//[Start]modified by asi466 on issue#0008402
		if($data['list'][0]['maxWdwlAmount']==NULL){
			foreach($data['list'] as $row => $value) {
				if($this->showCompCode($value['employee_id'])=="920"){				
					$data['list'][$row]['maxWdwlAmount'] = $this->Mloan_model->allowedWdwlForRetiree($value['employee_id']);				
				}
				else{				
	...
	{		
		...
		log_message('debug', "online_withdrawal param exist?:".array_key_exists('online_withdrawal',$_REQUEST));
		//[start] added by asi466 issue#5 google shared doc
		$_REQUEST["online_withdrawal"]["transaction_amount"] = str_replace(",","",$_REQUEST["online_withdrawal"]["transaction_amount"]);
		//[end] added by asi466 issue#5 google shared doc
		...
		if (array_key_exists('online_withdrawal',$_REQUEST)) {
			if($this->Transactioncode_model->transactionCodeExists($_REQUEST['online_withdrawal']['transaction_code'], 'CC')) {
				...
				//[start] google share#7 added by asi466 on 20120110
				if($saveOrSendFlag==1){
					$_REQUEST['online_withdrawal']['transaction_date'] = date("Ymd");
				}
				//[end] google share#7 added by asi466 on 20120110
				
				$this->Onlinecapitaltransactionheader_model->populate($_REQUEST['online_withdrawal']);
				//[START] Added by asi466 on issue#0008402
				$acctg_period = date("Ymd", strtotime($this->Parameter_model->getParam('ACCPERIOD')));
				$bal_info = $this->Mloan_model->showBalanceInfo($employee_id,$acctg_period);
				$this->Onlinecapitaltransactionheader_model->setValue('withdrawable_amount', $bal_info['maxWdwlAmount']);
				//[END] Added by asi466 on issue#0008402
				...
				if($transaction_code=="DDEP" &&
					...
					if($transaction_code=="WDWL") {
						...
						if($this->hasValidGuarantors($employee_id)){
							...
							$transaction_amount > $this->Mloan_model->allowedWdwlForRetiree($employee_id)){
				...
				if ($str_return == ""){
					...
					if($saveOrSendFlag==1){
						...
						//Added for PECA 6th Enhancement Kweeny Libutan 2012/05/10
						$this->Onlinecapitaltransactionheader_model->setValue('time_sent', date("His"));
					...
					if($result['affected_rows'] <= 0){
						...
						if($saveOrSendFlag==1){
							//[start] google share#7 added by asi466 on 20120110
							echo "{'success':true,'msg':'Data was successfully sent.','request_no':'".$_REQUEST['request_no']."'}";
							//[end] google share#7 added by asi466 on 20120110
						}
						else if($saveOrSendFlag==2){
							//[start] google share#7 added by asi466 on 20120110
							echo "{'success':true,'msg':'Data was successfully saved.','request_no':'".$_REQUEST['request_no']."'}";
							//[end] google share#7 added by asi466 on 20120110
						}
					}
				} else {
		...
		if (array_key_exists('online_withdrawal',$_REQUEST)) {
			if($this->Transactioncode_model->transactionCodeExists($_REQUEST['online_withdrawal']['transaction_code'], 'CC')) {
				...
				if($transaction_amount <= 0){
					$str_return = "{'success':false,'msg':'Amount must be greater than zero.','error_code':'8'}";
				}
				...
				if ($str_return == ""){
					...
					//Added for PECA 6th Enhancement Kweeny Libutan 2012/05/10
					$this->Onlinecapitaltransactionheader_model->setValue('time_sent', $saveOrSendFlag==1?date("His"):"");
	...
	*/
	...
	//	echo $checkDuplicate['error_message'];
		if($checkDuplicate['error_code'] == 1){
			...
			$this->addDtl($trans_no, $capcon['transaction_amount'], $capcon['created_by'],$capcon['employee_id']);
	...
	function addDtl($trans_no, $amt, $user,$employee_id = null){
		...
		if($_REQUEST['capcon']['transaction_code']=="WDWL" && (($this->noOfWithdrawals($trans_no,$employee_id))>$allowed_withdrawals)){
	...
	//[start]20111121 added by asi466 issue:0008377
	function noOfWithdrawals($trans_no=null,$employee_id){
	...
	//[end]20111121 added by asi466 issue:0008377
}
?>
