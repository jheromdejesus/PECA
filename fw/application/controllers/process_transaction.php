...
//		$this->processTransactions();
	...
	function processTransactions(){
		...
		##### NRB START EDIT #####
		$transactions = json_decode(isset($pt_array['data']) ? stripslashes($pt_array['data']): '', true);
				...
				try{
					...
					foreach ($cap_trans as $row){
			    		...
			    		if ($tran_code == 'WDWL'){
							...
							log_message('debug', 'allowed_amount:'.$allowed_amount);
							
							$tran_amount = round($tran_amount,2);
							log_message('debug', 'allowed_amount:'.$tran_amount);
...
//					$this->db->trans_complete();
		...
		if ($npml == 1){
			try{
				...
				foreach ($npml_employees as $row){
					$employee_id = $row['employee_id'];
					$employee_loan_no = $row['loan_no'];
					//do not penalize employee if he already has paid but late
					if($row['charged']=='true'){
					...
					$tblsuspension_result = $this->tblsuspension_model->get(array('employee_id'=>$employee_id, 'loan_no'=>$employee_loan_no, 'suspension_type' => 'NPML'));
					if($tblsuspension_result['count']==0){
						$tblsuspension_param['employee_id'] = $employee_id; //set insert values
						$tblsuspension_param['due_date'] = $date; 
						$tblsuspension_param['loan_no'] = $employee_loan_no; 
					...
					$tblnpmlsuspension_result = $this->tblnpmlsuspension_model->get(array('employee_id'=>$employee_id, 'loan_no'=>$employee_loan_no, 'suspension_type' => 'NPML','suspended_date'=>$next_acctg));
					...
					if($tblnpmlsuspension_result['count']==0){ //commented to keep track of the members who are suspended under npml of the same acct period
						$tblnpmlsuspension_param['employee_id'] = $employee_id; //set insert values
						$tblnpmlsuspension_param['due_date'] = $date; 
						$tblnpmlsuspension_param['loan_no'] = $employee_loan_no; 
	...
	function getWithrawableAmount($employee_id, $acctg_period){
		...
		//[START] 8th Enhancement MODIFIED BY VINCENT SY 20130814
		$temp_capcon_req = $this->Mloan_model->getEmployeeBalanceInfo($employee_id,$acctg_period);	
		//[END] 8th Enhancement MODIFIED BY VINCENT SY 20130814
		$capcon_req = $temp_capcon_req['maxWdwlAmount'];
		log_message('debug', 'bef$req = ' . $capcon_req);
		
		//get employee's capcon balance
		$capcon_bal = $this->Capitalcontribution_model->retrieveCapConBalance($employee_id, $acctg_period);	
		log_message('debug', 'bef$capcon_bal = ' . $capcon_bal);	
		$req = ($capcon_req >= $min_bal) ? $capcon_req : $min_bal;
		
		log_message('debug', 'a$req = ' . $capcon_req);
		log_message('debug', 'aft$capcon_bal = ' . $capcon_bal);
		log_message('debug', '$emp id = ' . $employee_id);
		
		// return $capcon_bal - $req; asi466 20130322
		return $capcon_req;
	}
	...
	function saveInvestment($row, $date, $acctg_period, $journal_no, $user, $new = true){
		...
		while ($bplacement_date <= $maturity_date){
			...
			/* START Modified for Disbalance in JV Issue by ASI372 2013/05/16 */
			$temp_amount = round((($tran_amount * $interest * $diff) / 36000),2);
			log_message('debug', 'saveInvestment:' . $tran_amount ." ". $interest. " ".$diff." ".$temp_amount);	
			/* END Modified for Disbalance in JV Issue by ASI372 2013/05/16 */
