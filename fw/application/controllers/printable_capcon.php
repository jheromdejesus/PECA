...
class Printable_capcon extends Asi_Controller 
{
	...
	{	
		...
		foreach($this->list AS $data) 
		{
			...
			$objPdf->Cell(0,0,"10th Floor Net Park, 5th Avenue, Cresent Park West, Bonifacio Global City, Taguig 1634","","","C",false);
			$objPdf->Ln(.4);
			$objPdf->SetFont('Arial','',9);
			$objPdf->SetX(6);
			...
			$objPdf->Ln(.25);
			$objPdf->SetX(1);
			//jdj 07072017: remove Loan Balance
			...
			//jdj 07072017: remove remaining loan balance in this pdf as requested
	...
	{	
		...
		if($this->transaction_code=='CLSE') {
			$this->non_withdrawable = 0; 
		}
		else {
		$this->non_withdrawable = $result['reqBal']; 
		}
