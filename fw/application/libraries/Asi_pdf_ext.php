...
class Asi_pdf_ext 
{
	...
	var $next_line_on_overflow = false;
	var $overflow_row_index = "";
	var $split_length = 40;
	var $removeHeader2LastChar = false;
	var $count = 0;
	...
	//[START] Added by Vincent Sy for 8th Enhancement 2013/07/04
	/**
	 * @param $transaction_description
	 ...
	 * @param $col_start2
	 * @param $font_size
	 * to display principal amount in loan app prooflist
	 */
	function writePrincipalAmount($transaction_description, $principal_amount, $col_start=.7, $col_start2=2,$font_size = 8) 
	{
		//this to ensure that subheader in a page will have atleast 1 row of data
		if ($this->i==$this->row_count - 1) {
			$this->Ln(null, false);
		...
		$this->col_start = $col_start;
		$this->pdf_obj->SetX($this->col_start);
		$this->pdf_obj->SetFont('Arial','',$font_size);
		$this->pdf_obj->Cell($this->row_height,$this->row_height,$transaction_description,'','','L');
		$this->col_start = $col_start2;
		$this->pdf_obj->SetX($this->col_start);
		$this->pdf_obj->SetFont('Arial','B',9);
		$this->pdf_obj->Cell($this->row_height,$this->row_height,$principal_amount,'','','R');
	}
	//[END] Added by Vincent Sy for 8th Enhancement 2013/07/04
	...
	function CustomizedSubheader() 
	{
		...
		foreach($this->headers2 AS $label => $width) {
			...
			if($this->removeHeader2LastChar){ //useful for removing excess C or D in header without altering logic
				$new_label = substr($label, 0, strlen($label)-1);
			}
			if ($this->align2==null) {
				if($this->fit==null)
					$this->pdf_obj->Cell($width*$this->row_height,$this->row_height,$new_label,'','','C',true);
				else
					$this->pdf_obj->CellFitScale($width*$this->row_height,$this->row_height,$new_label,'','','C',true);			
			}
			else {
				if($this->fit==null)
					$this->pdf_obj->Cell($width*$this->row_height,$this->row_height,$new_label,'','',$this->align2[$label],true);
				else
					$this->pdf_obj->CellFitScale($width*$this->row_height,$this->row_height,$new_label,'','',$this->align2[$label],true);
			}	
		}
		$this->Ln();
	...
	function writeTableData($data, $width, $alignment, $col_start=.7, $ln = null, $row_count = null, $fontsize=7, $with_line = false, $with_line_basis = "") 
	{
		...
		foreach($data AS $row) {
			...
			//used for columns whose contents do not fit horizontally so it is shifted down
			if($this->next_line_on_overflow){
				$value_arr = str_split($row[$this->overflow_row_index], $this->split_length);
				$row[$this->overflow_row_index] = $value_arr[0];
			}
			...
			if($this->next_line_on_overflow){
				...
				foreach($value_arr as $value_str){	
					if($index==0){ //exclude first, since it is already displayed above
						$index = $index+1;	
						continue;		
					}									
					$this->Ln($ln);
					$this->pdf_obj->Cell($width[$this->overflow_row_index]*$this->row_height,$this->row_height,$value_str,'','',$alignment[$this->overflow_row_index]);	
					$index = $index+1;
	...
	function writeAmortHeaderInfo($amort_header_info) 
	{
		$this->report_range = null;
		
		$this->loan_period = $amort_header_info["loan_period"];
		$this->run_date = $amort_header_info["run_date"]; 
		$this->report_title = $amort_header_info["report_title"];
		$this->employee_id = $amort_header_info["employee_id"];
		$this->employee_name = $amort_header_info["employee_name"];
		$this->loan_amount = $amort_header_info["loan_amount"];
		$this->annual_contractual_rate = $amort_header_info["annual_contractual_rate"];
		$this->terms = $amort_header_info["terms"];
		$this->other_charges = $amort_header_info["other_charges"];
		$this->principal_amount = $amort_header_info["principal_amount"];
		$this->initial_interest = $amort_header_info["initial_interest"];
		$this->service_charge = $amort_header_info["service_charge"];
		$this->other_charges_amount = $amort_header_info["other_charges_amount"];
		$this->loan_proceeds = $amort_header_info["loan_proceeds"];
		$this->total_interest_paid = $amort_header_info["total_interest_paid"];
		$this->pdf_obj->AliasNbPages();
		$this->AddPage();
		$this->AmortHeader();
	}
	
	function AmortHeader() 
	{
		$this->pdf_obj->SetFont('Arial','I',9);
		$this->pdf_obj->Cell(0, 0, $this->loan_period, "", "", "C", false);
		$this->Ln(0.4);
		
		$this->pdf_obj->SetFont('Arial','B',9);
		$this->pdf_obj->Cell(23*$this->row_height, $this->row_height, "PECA Savings and Loans Monitoring System");
		$this->Ln();			
		$this->pdf_obj->SetFont('Arial','B',8);
		$this->pdf_obj->Cell(23*$this->row_height, $this->row_height, $this->report_title);
		$this->pdf_obj->SetFont('Arial','',9);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, "Run Date: ".$this->run_date);
		$this->Ln(0.3);
		
		$this->pdf_obj->Cell(23*$this->row_height, $this->row_height, "Employee ID:                 ".$this->employee_id);
		$this->Ln();
		$this->pdf_obj->Cell(23*$this->row_height, $this->row_height, "Name:                            ".$this->employee_name);
		$this->Ln(0.3);
		
		$this->pdf_obj->Cell(8*$this->row_height, $this->row_height, "Loan Amount (pv)", "", "", "R", false);
		$this->pdf_obj->Cell(7*$this->row_height, $this->row_height, $this->loan_amount, "", "", "R", false);
		$this->pdf_obj->SetX(4.5);
		$this->pdf_obj->Cell(2*$this->row_height, $this->row_height, "Initial Interest", "", "", "L", false);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, $this->initial_interest, "", "", "R", false);
		$this->Ln();
		$this->pdf_obj->Cell(8*$this->row_height, $this->row_height, "Annual Contractual Rate", "", "", "R", false);
		$this->pdf_obj->Cell(7*$this->row_height, $this->row_height, $this->annual_contractual_rate, "", "", "R", false);
		$this->pdf_obj->SetX(4.5);
		$this->pdf_obj->Cell(2*$this->row_height, $this->row_height, "Service Charge", "", "", "L", false);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, $this->service_charge, "", "", "R", false);
		$this->Ln();
		$this->pdf_obj->Cell(8*$this->row_height, $this->row_height, "terms (Nper)", "", "", "R", false);
		$this->pdf_obj->Cell(7*$this->row_height, $this->row_height, $this->terms, "", "", "R", false);
		$this->pdf_obj->SetX(4.5);
		$this->pdf_obj->Cell(2*$this->row_height, $this->row_height, "Other Charges Amount", "", "", "L", false);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, $this->other_charges_amount, "", "", "R", false);
		$this->Ln();
		$this->pdf_obj->Cell(8*$this->row_height, $this->row_height, "Other Charges", "", "", "R", false);
		$this->pdf_obj->Cell(7*$this->row_height, $this->row_height, $this->other_charges, "", "", "R", false);
		$this->pdf_obj->SetX(4.5);
		$this->pdf_obj->Cell(2*$this->row_height, $this->row_height, "Loan Proceeds", "", "", "L", false);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, $this->loan_proceeds, "", "", "R", false);
		$this->Ln();
		$this->pdf_obj->Cell(8*$this->row_height, $this->row_height, "Principal Amount", "", "", "R", false);
		$this->pdf_obj->SetFont('Arial','B',9);
		$this->pdf_obj->Cell(7*$this->row_height, $this->row_height, $this->principal_amount, "", "", "R", false);
		$this->pdf_obj->SetFont('Arial','',9);
		$this->pdf_obj->SetX(4.5);
		$this->pdf_obj->Cell(2*$this->row_height, $this->row_height, "Total Interest Paid", "", "", "L", false);
		$this->pdf_obj->Cell(14*$this->row_height, $this->row_height, $this->total_interest_paid, "", "", "R", false);
		$this->Ln(0.6);
	}
	
	function writeAmortSchedSubheader($initial_balance, $initial_cashflow) 
	{
		$this->initial_balance = $initial_balance;
		$this->initial_cashflow = $initial_cashflow;
		$this->AmortSchedSubheader();
	}
	
	function AmortSchedSubheader() 
	{
		$this->pdf_obj->SetFillColor(60, 60, 60);
		$this->pdf_obj->SetTextColor(255, 255, 255);
		$this->pdf_obj->SetFont('Arial','',7);
		$this->pdf_obj->Cell(5.602, 0.15, $this->initial_balance,'','','R',true);
		$this->pdf_obj->Cell(1.24, 0.15, $this->initial_cashflow,'','','R',true);
		$this->Ln();
		$this->pdf_obj->SetTextColor(0, 0, 0);
		$this->pdf_obj->SetFillColor(255, 255, 255);
	}
	
	function writeMIR($MIR) 
	{
		$this->Ln();
		$this->Ln();
		$this->pdf_obj->SetFillColor(255, 255, 255);
		$this->pdf_obj->SetFont('Arial','',9);
		$this->pdf_obj->Cell(2.7, 0.15, "Monthly Interest Rate (MIR)",'','','L',true);
		$this->pdf_obj->Cell(2.7, 0.15, $MIR,'', '','L',true);
	}
	
	function writeEIR($EIR) 
	{
		$this->Ln(0.2);
		$this->pdf_obj->SetFillColor(255, 255, 255);
		$this->pdf_obj->SetFont('Arial','',9);
		$this->pdf_obj->Cell(2.7, 0.15, "Effective Interest Rate (EIR)",'','','L',true);
		$this->pdf_obj->Cell(2.7, 0.15, $EIR,'', '','L',true);
	}
	//[END] 7th Enhancement
}
